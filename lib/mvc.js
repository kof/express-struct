...
function mapSocket(server, o) {
    var socketConnection = socket.listen(server, o.socket);
...
function mapSocket(server, o) {
    ...
    function Request(client, msg) {
        ...
        this.query = queryStr ? querystring.parse(queryStr) : {};           
        this.method = msg.method || this.method;
        this.headers = msg.headers || headers;
        this.url = msg.url;
        this.body = msg.body;
    }

    socketConnection.on('connection', function(client) {
        Request.prototype = client.request;
        
        client.on('message', function(msg) {
            var res, req;

            req = new Request(client, msg);
