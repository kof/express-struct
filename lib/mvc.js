 ...
 *
 * @param {String} path to the root dir.
 */
function cacheControllers(root) {
    findit.sync(root).forEach(function(path, stat) {
        if (/\.js$/.test(path)) {
            require(path);
        }
    });
 ...
 *
 * @param {Error|String} error or message
 * @param {String} path which has triggered the issue.
 * @return {Error}
 */
function error(err, path) {
...
exports.setup = function(server, options) {
    ...
    function handler(req, res, next) {
        ...
        if (!module) {
            return next(error('Controller not found.', controllerPath));
        }
        ...
        if (!actionName) {
            ...
            actionName = controller.CRUD ? methodsMap[req.method] : p.controller;
        }

        if (!controller[actionName]) {
            return next(error('Action not found.', controllerPath + ': ' + actionName));
        }

        try {
            controller[p.action](req, res, next);
        } catch(err) {
            next(error(err), controllerPath + ': ' + actionName);
        }
    }

    cacheControllers(o.root);
